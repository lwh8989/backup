...
function sliderInit() {
  ...
  for (var i = 0; i <= bannerLen - 1; i++) {
    ...
    $('.slider--shape-i .slider__bg' + ' .bg' + i).css('backgroundColor', bgColorArray[i])
  }
  $('.slider--shape-i .slider__nav li').css('width', +bannerWithArray[$('.slider--shape-i .slider__nav li').length - 1] + 'px')
...
UIAPP.gallery = {
  ...
  bindings: function(options) {
    ...
    $thumnail.parent().css('width', $thumnail_itemPer * options._thumbnalilWidth + 'px')
    $(options._screen)
      .find('iframe')
      .remove()
    ...
    $(options._el)
      ...
      .click(function() {
        ...
        $(options._screen)
          .find('iframe')
          .remove()
        $(options._screen).append('<iframe width="' + $(options._screen).width() + '" height="' + $(options._screen).height() + '"  src="' + $(this).attr('href') + '" frameborder="0" allowfullscreen></iframe>')
        return false
      })
    ...
    $(options._thumControler)
      ...
      .click(function() {
        if ($thumnail_currentPage < $thumnail_page) {
          ...
          $thumnail.animate(
            {
              marginLeft: $thumnail_itemPer * options._thumbnalilWidth * $thumnail_currentPage * -1
            },
            500
          )
        }
...
UIAPP.rateIt = {
  ...
  bindings: function(options) {
    ...
    options._outTarget.on('mouseover', function(e) {
      if (!options._outTarget.attr('mouse') || options._outTarget.attr('mouse') !== 'out') {
        return false
      }
...
UIAPP.slider = {
  ...
  bindings: function(options) {
    ...
    if (options._slider_randomPlay == true) {
      var $curSlide = Math.floor(Math.random() * options._screen.find('ul').length + 0)
    } else {
      var $curSlide = 0
    }
    ...
    if (!options._slider_dot) {
      ...
      options._paging.find('.slider__num').append("<div class='current__num'></div>")
      options._paging.find('.slider__num').append("<div class='end__num'></div>")
    ...
    function setSlider(num, pos) {
      ...
      if (pos == 'left') {
        ...
        options._screen
          ...
          .eq(num)
          .stop()
          .animate({ left: 0 }, options._slider_speed, 'easeInOutQuart', function() {})
        options._screen
          .find('ul')
          .eq($oldSlide)
          .stop()
          .animate({ left: $slideWidth * -1 }, options._slider_speed, 'easeInOutQuart', function() {})
      }
      if (pos == 'right') {
        options._screen
          .find('ul')
        ...
        options._screen
          ...
          .eq($oldSlide)
          .stop()
          .animate({ left: $slideWidth }, options._slider_speed, 'easeInOutQuart', function() {
            options._screen
              .find('ul')
              .eq($tempSlide)
              .css('left', $slideWidth * -1 + 'px')
    ...
    $selectBoxes
      .each(function(i, select) {
        ...
        $select
          ...
          .addClass(wrapClassList.join(' '))
          .addClass(shape)
          .prepend('<strong class="' + titleClassName + '">' + select.options[select.selectedIndex].text + '</strong>')
...
UIAPP.popover = {
  ...
  bindings: function(options) {
    ...
    ;(name = options.name || 'popover--tooltip'), (title = ''), (content = '')
    ...
    if (options.event == undefined) {
      if (options.autoHide == undefined || options.autoHide == true) {
        $popover.delay(options.delay || 3000).fadeOut(function() {
          $(this).remove()
          options.callback && options.callback()
        })
      }
...
UIAPP.alert = {
  ...
  markup: function(values) {
    ...
    if (values.isConfirm) {
      html += '<button class="button--base-c button--shape-a js-confirm" type="button">' + this.i18n[lang].confirm + '</button>'
      html += '<button class="button--base-c button--shape-b js-cancel" type="button">' + this.i18n[lang].cancel + '</button>'
    } else {
      html += '<button class="button--base-c button--shape-a js-alert-x" type="button">' + this.i18n[lang].close + '</button>'
    }
    html += '<button class="dialog__x js-alert-x" type="button">' + this.i18n[lang].close + '</button>'
    html += '</footer>'
    html += '</div>'
    return html
  },
  ...
  addImg: function() {
    var _this = this,
      ...
      IMG_TEMPLATE = function(imgSrc) {
        return '<img src="' + imgSrc + '" class="' + itemClassName + '" alt="">'
      }
...
UIAPP.cardSlider = {
  ...
  init: function(options) {
    ...
    for (var i = 0; i < slider.length; i++) {
      ...
      this.bindings({
        _el: slider.eq(i),
        _auto: rolling,
        _delay: slider.eq(i).attr('delay') != undefined ? Number(slider.eq(i).attr('delay')) : null,
        _paging: paging,
        _button: button,
        _index: i,
        _animate: animate
  ...
  bindings: function(options) {
    ...
    if (options._button) {
      slider.append('<button type="button" class="card__prev"></button><button type="button" class="card__next"></button>')
    }
    ...
    for (var i = 0; i < item.length; i++) {
      if (i) {
        ...
        if (options._paging) {
          paging.append('<button type="button" class="card__paging__control"></button>')
        }
      } else {
        if (options._paging) {
          paging.append('<button type="button" class="card__paging__control is-active"></button>')
        }
      }
    }
...
function getOffsetBody() {
  if ($('#getOffsetBody').length == 0) {
    $('body').append('<div style="position:fixed;top:0;left:0;width:100%;height:100%;z-index:-1;" id="getOffsetBody"></div>')
  }
...
function scrollUI(arr) {
  ...
  this.track.on('mousedown', function(e) {
    ...
    self.body.scrollTop((scHeight / (self.track.height() - self.handle.height())) * (new getMouse(e).Y - self.handle.height() / 2 + $(window).scrollTop() - self.track.offset().top))
  })
  this.topBtn.on('mousedown', function() {
    clearInterval(self.scrollLiveClear)
    var scrTop = self.scrollBtmStop(self.body.scrollTop() - (self.getContentHeight(self.body) - self.body.height()) / 10)
  ...
  function wheel(detail) {
    if (detail < 0) {
      clearInterval(self.scrollLiveClear)
      scrTop = self.scrollBtmStop(self.body.scrollTop() + (self.getContentHeight(self.body) - self.body.height()) / 20)
    } else {
      scrTop = self.scrollBtmStop(self.body.scrollTop() - (self.getContentHeight(self.body) - self.body.height()) / 20)
    }
    self.body.scrollTop(scrTop)
...
scrollUI.prototype.setup = function() {
  ...
  html += '<button type="button" class="scrollable-arrow-top" disabled="disabled"><i class="icon icon--arrow-up"></i></button>'
  html += '<div class="scrollable-track">'
  html += '<button class="scrollable-face" type="button"></button>'
  html += '</div>'
  html += '<button type="button" class="scrollable-arrow-bottom"><i class="icon icon--arrow-down"></i></button>'
  html += '</div>'
  html += '</div>'
...
scrollUI.prototype.scrollLive = function() {
  ...
  this.scrollLiveClear = setInterval(function() {
    if (self.body[0].offsetHeight + 10 >= self.getContentHeight(self.body) || self.body[0].offsetHeight > self.getContentHeight(self.body)) {
...
scrollUI.prototype.buttonPosition = function() {
  ...
  this.body.scrollTop() / (scHeight / (this.track.height() - this.handle.height()))
  this.handle.css('top', Math.ceil(this.body.scrollTop() / (scHeight / (this.track.height() - this.handle.height()))))
}
scrollUI.prototype.scrollContent = function() {
  var scHeight = this.getContentHeight(this.body)
  scHeight = scHeight - this.body.height()
  this.body.scrollTop((scHeight / (this.track.height() - this.handle.height())) * this.handle.position().top)
}
...
UIAPP.viewer = {
  ...
  add: function(data, index) {
    ...
    this.html += '<div class="spinner spinner--base-a" id="spinner-media"></div>'
    if (this.data.length > 1) {
      this.html += '<button type="button" class="media__view-prev" id=""><i class="icon icon--prev"></i></button>'
      this.html += '<button type="button" class="media__view-next" id=""><i class="icon icon--next"></i></button>'
    }
    this.html += '</div>'
    ...
    if (this.data.length > this.pageing) {
      this.html += '<button type="button" class="media__thumbnail-prev" style="display:none"><i class="icon icon--prev"></i></button>'
      this.html += '<button type="button" class="media__thumbnail-next" style="display:none"><i class="icon icon--next"></i></button>'
    }
    this.html += '</div>'
    this.html += '<div class="media__paging">'
    this.html += '<strong class="current">1</strong>  /   <span class="total">4</span>'
    this.html += '</div>'
    this.html += '<button title="close" class="btn media__close" type="button">'
    this.html += '<i class="icon icon--close"></i>'
    this.html += '</button>'
    this.html += '<div class="media__modal"></div>'
    this.html += '</div>'
    this.html += '<div id="imageload" style="position:absolute;top:-1000px;left:-1000px;width:100px;height:100px;overflow:hidden;"></div>'
    if ($('#media').length == 0) {
    ...
    this.mediaLayer.find('.scrollable-arrow-top , .scrollable-face , .scrollable-arrow-bottom ').on('click', function(e) {
      e.stopPropagation()
    })
  },
  init: function() {
    ...
    for (var i = 0; i < this.data.length; i++) {
      ...
      if (data.img_width && data.img_height) {
        html = '<li class="media__thumbnail__items__item"><div class="spinner spinner--base-a"></div><a href="' + mediaUrl + '" class="media-item media-' + mediaType + ' ' + activeClassName + '" media-index="' + i + '"><img src="' + thumbUrl + '" id="item-' + i + '"  media-width="' + data.img_width + '" media-height="' + data.img_height + '" />' + videoicon + '</a></li>'
      } else {
        html = '<li class="media__thumbnail__items__item"><div class="spinner spinner--base-a"></div><a href="' + mediaUrl + '" class="media-item media-' + mediaType + ' ' + activeClassName + '" media-index="' + i + '"><img src="' + thumbUrl + '" id="item-' + i + '"  />' + videoicon + '</a></li>'
      }
  ...
  insertResizeItem: function(n, func) {
    ...
    if (mediaType == 'image') {
      videoicon = ''
      html = '<a href="' + mediaUrl + '" class="media-item media-' + mediaType + '" media-type="image" media-index="' + n + '" target="_blank"><img id="item-resize-' + n + '" src="' + mediaUrl + '" org-width="' + (orgWidth == undefined ? 676 : orgWidth) + '" org-height="' + (orgHeight == undefined ? 360 : orgHeight) + '" id="item-' + n + '" />' + videoicon + '</a>'
    } else {
      html = '<div class="media-item" media-index="' + n + '"><iframe width="100%" height="100%" src="' + mediaUrl + '" frameborder="0" allowfullscreen id="item-resize-' + n + '" org-width="' + 640 + '" org-height="' + 480 + '" minetype="video"></iframe></div>'
    }
  ...
  viewSlider: function() {
    ...
    function imgonload(dir, n) {
      ...
      if ($('#item-resize-' + n).attr('minetype') && $('#item-resize-' + n).attr('minetype') == 'video') {
  ...
  viewsResize: function(anicheck, n) {
    ...
    var resize = $('#resize')
    var small = 1.1
    var ratio = this.resizeWidth > this.resizeHeight ? this.resizeWidth / this.resizeHeight : this.resizeHeight / this.resizeWidth
    self.imageLoad.html('')
    ...
    function resizingImage(a) {
      ...
      if (self.resizeWidth < bodyWidth) {
        ...
        resizeHeight = self.resizeWidth > self.resizeHeight ? self.resizeWidth / ratio : resizeWidth * ratio
        if (resizeHeight > bodyHeight) {
          resizeHeight = bodyHeight
          resizeWidth = self.resizeWidth > self.resizeHeight ? resizeHeight * ratio : resizeHeight / ratio
        }
      }
      if (self.resizeHeight < bodyHeight) {
        ...
        if (resizeWidth > bodyWidth) {
          resizeWidth = bodyWidth
          resizeHeight = self.resizeWidth > self.resizeHeight ? resizeWidth / ratio : resizeWidth * ratio
        }
      }
...
spinner.prototype.loading = function() {
  ...
  for (var i = 0; i <= 11; i++) {
    ...
    style += '{'
    style += 'animation: spinner--base-d-keyframe 1.1s infinite;animation-delay:' + i / 10 + 's;'
    style += '}'
  }
  if ($('#spinner--base-d').length == 0) {
    $('head').append('<style type="text/css" id="spinner--base-d-style"></style>')
...
spinner.prototype.complete = function() {
  ...
  this.gif.html('<span class="spinner__complte"><i class="icon icon-check"></i></span>')
...
spinner.prototype.setup = function(set) {
  ...
  for (var i = 0; i <= 11; i++) {
    ...
    left = left.toFixed(3)
    top = top.toFixed(3)
    data += '<span class="item item-' + i + '" style="left:' + left + 'px;top:' + top + 'px;"><span></span></span>'
  }
  data += '<div>'
  return data
}
...
sliderContent.prototype.init = function() {
  ...
  for (var i = 0; i < this.items.length; i++) {
    if (!i) {
      html += '<a href="" class="is-active"></a>'
    } else {
      html += '<a href=""></a>'
    }
  }
  if (sliderContentClear) {
    clearInterval(sliderContentClear)
  }
  this.clear = null
  this.paging.html(html)
  this.paging_item = this.paging.find('a')
  this.move_current_left = null
  this.move_prev = null
  this.paging_item.unbind('click')
  ...
  this.btn_prev.unbind('click')
  this.btn_next.unbind('click')
  this.ia_animate = null
  this.obj.on('mouseenter', function() {
    clearInterval(sliderContentClear)
  })
  this.obj.on('mouseleave', function() {
    setInterval(sliderContentClear)
    self.rolling()
  })
  this.paging_item.on('click', function(e) {
    ...
    $(this).addClass('is-active')
    if (prev != $(this).index()) {
      self.animation(prev, $(this).index())
    }
  ...
  this.btn_prev.on('click', function(e) {
    ...
    self.move_current_left = 0
    self.move_prev = -400

    self.paging_item.removeClass('is-active')
    self.paging_item.eq(self.counter).addClass('is-active')
...
sliderContent.prototype.rolling = function() {
  var self = this
  clearInterval(sliderContentClear)
  sliderContentClear = setInterval(function() {
    actions()
  }, 3600)
  function actions() {
    var prev = self.counter
...
sliderContent.prototype.animation = function(p, c) {
  var self = this
  this.ia_animate = true
  var prev = p
  var current = c
  var currentLeft = self.move_current_left
  var prevLeft = self.move_prev
